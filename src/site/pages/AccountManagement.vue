...
const loadAccounts = async () => {
 try {
 const data = await browserStorage.get('fb_control_accounts');
...
const saveAccount = async () => {
 ...
 if (index !== -1) {
 accounts.value[index] = { ...newAccount, createdAt: accounts.value[index].createdAt };
 }
 }
 else {
 accounts.value.push(newAccount);
 }
 await browserStorage.set('fb_control_accounts', JSON.stringify(accounts.value));
 resetForm();
...
const filterAccounts = () => {
 ...
 filteredAccounts.value = accounts.value.filter(a => a.name.toLowerCase().includes(query) ||
